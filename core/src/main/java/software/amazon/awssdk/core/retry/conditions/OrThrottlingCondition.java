 ...
 */
@SdkPublicApi
public class OrThrottlingCondition implements ThrottlingCondition {

    private List<ThrottlingCondition> conditions = new ArrayList<>();

    public OrThrottlingCondition(ThrottlingCondition... conditions) {
    ...
    public boolean isThrottlingException(Exception context) {
        return conditions.stream().anyMatch(retryCondition -> retryCondition.isThrottlingException(context));
    }
}
