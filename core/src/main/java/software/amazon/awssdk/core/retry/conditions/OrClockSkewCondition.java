 ...
 */
@SdkPublicApi
public class OrClockSkewCondition implements ClockSkewCondition {

    private List<ClockSkewCondition> conditions = new ArrayList<>();

    public OrClockSkewCondition(ClockSkewCondition... conditions) {
    ...
    public boolean isClockSkewError(Exception exception) {
        return conditions.stream().anyMatch(r -> r.isClockSkewError(exception));
    }
}
